...
<el-table
  ...
  <el-row v-if="pathURL=='pay'">
        <el-input v-model="money" placeholder="请输入预支付金额"></el-input>
        <el-button @click="payMoney()" type="success" round>确认付款</el-button>
  </el-row>
  <el-row>
...
export default {
  ...
  methods: {
        ...
        payMoney(){
        this.$http.get(this.$dynamicHref+"/order/pay/"+this.orderId+"/"+this.money).then((res)=>{
        if(res.data==true){
      ...
      this.exitBack();
        ...
        exitBack(){
            this.$router.push("/userOrder?pageNo="+this.pageNo+"&pageSize="+this.pageSize);
        },
        toCN(completeSatus){
        var action = "";
