...
export default {
  ...
  methods: {
    submitForm() {
      ...
      // 假接口，测试用
     // Vue.prototype.$logUser = userAccount;
      //this.$globalEventBus.$emit("styleChange", "auto", this.$logUser.type);
      //this.$router.push("/home");
      //this.$globalEventBus.$emit("indexChange", "2");
      ...
      var user = {};
       (user["id"] = userAccount),
         ...
         this.$http
           .post(this.$dynamicHref + "/admin/logIn", user)
           .then((res) => {
             if (!res.data) {
               alert("用户名或者密码错误，登陆失败");
             } else {
               Vue.prototype.$logUser = res.data;
